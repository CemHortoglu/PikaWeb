...
1. **Orphan Legacy Pricing View (`Views/Home/Pricing.cshtml`):**
   - **Evidence Level:** `A. CODE_VERIFIED`
   - **Governance Status:** `LEGACY / DO_NOT_MARKET`
   - Contains hardcoded pricing packages (`₺9.900/ay`, `₺24.900/ay`, `Start / Growth / Enterprise`), features matrix, and SLA claims. Has **NO** controller action in `HomeController.cs`. Must remain strictly unrouted and is scheduled for archival/removal from public routing in Phase P01.
...
- Support / service scope (SLA response tiers, dedicated account manager)
- Contract terms
...
- **Do not expose legacy `Views/Home/Pricing.cshtml` values.**
