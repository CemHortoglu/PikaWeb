...
- **Evidence Level:** `A. CODE_VERIFIED`
- **Defect:**
  - `Views/Home/Pricing.cshtml`: Contains hardcoded prices (`₺9.900/ay`, `₺24.900/ay`, `Enterprise Özel`) and enterprise SLA claims. Has no action in `HomeController.cs`. Strictly classified as **`LEGACY / DO_NOT_MARKET`**. Pika operates on a non-negotiable quotation-based commercial policy (*"İhtiyacınıza ve kullanım kapsamınıza göre özel teklif"*).
...
- **Remediation:** In Phase P01, remove or archive `Pricing.cshtml` from public routing, delete `JourneyManager.cshtml.bak`, and archive `Solutions.cshtml`. Ensure no fixed package prices or calculators are ever exposed publicly.
