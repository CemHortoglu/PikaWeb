...
4. Başarılı login'den sonra backend, Angular login endpoint'ine otomatik form post yapar:
   - `POST http://localhost:4200/login` (hedef URL, `Auth:AngularAppUrl` üzerinden `/login` olarak üretilir)
   - Form alanı: `authResponse`
   - `authResponse` içeriği: API login cevabının JSON string hali (`token.accessToken`, `token.refreshToken`, `token.expiration`, `token.refreshTokenExpiration`, `languageCode`).
5. Login sayfasına tekrar girildiğinde mevcut oturum kontrolü ile otomatik redirect yapılmaz; kullanıcı istediği zaman farklı hesapla tekrar login olabilir.
6. Cross-site kullanım için cookie ayarı `SameSite=None`, `Secure`, `HttpOnly` olacak şekilde yapılandırılmıştır.
7. Angular origin'i için CORS policy (`https://app.publish.tr`) ve credentials desteği açıktır.
8. Auth API endpoint'leri:
...
### Adım 1: Login endpoint'ininde post karşılayın
- Angular tarafında `POST /login` isteğini karşılayacak bir giriş noktası tanımlayın.
- Beklenen request body formatı (`application/x-www-form-urlencoded`):
  - `authResponse=<JSON_STRING>`
- Angular'ın beklediği parse edilmiş örnek payload:
```json
{
  "token": {
    "accessToken": "...",
    ...
    "refreshTokenExpiration": "2026-01-15T10:00:00Z"
  },
  "languageCode": "tr"
}
```

### Adım 2: Angular response beklentisi
- Bu istek için backend herhangi bir response beklemez (tarayıcı submit'i tek yönlüdür).
- Angular tarafı başarılı işlediğinde kullanıcıyı kendi akışına göre yönlendirmelidir (örn. `/admin`, `/dashboard`).
- Minimum beklenti: HTTP `200` veya yönlendirme (`302`) ile akışın devam etmesi.

### Adım 3: Ortam URL ayarları
- Angular environment içinde ZBIM web API host'u net tanımlansın.
  - Örnek: `authBaseUrl = "https://publish.tr"`

### Adım 4: HttpClient credentials
- `authBaseUrl` altındaki auth çağrılarında `withCredentials: true` zorunlu olmalı.
- Öneri: bir interceptor ile otomatik ekleyin.

### Adım 5: Uygulama açılış bootstrap
...
### Adım 7: Login component revizyonu
...
2. Login başarılı -> Angular `POST /login` isteğini alır ve kullanıcıyı uygulama içine taşır.
